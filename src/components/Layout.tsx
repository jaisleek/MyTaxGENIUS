...
export default function Layout() {
  ...
  const mainNav = [
    ...
    { name: 'Tax Library', path: '/blog', icon: BookOpen },
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-slate-950 flex flex-col font-sans relative selection:bg-emerald-200 selection:text-emerald-900 transition-colors duration-300">
      {/* Soft Friendly Security Banner */}
      <div className="bg-emerald-50 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 text-xs py-2 px-4 text-center flex justify-center items-center font-bold tracking-wide">
        <Shield className="w-3.5 h-3.5 mr-1.5 text-emerald-600 dark:text-emerald-400" />
        Safe & Secure • NDPR Compliant • Data Encrypted
      </div>

      {/* Modern Clean Header */}
      <header className="bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl sticky top-0 z-40 transition-all duration-300 shadow-[0_4px_30px_rgb(0,0,0,0.03)] border-b border-white dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            
            {/* Logo */}
            <Link to="/" className="flex items-center space-x-3 group cursor-pointer z-50 mr-8">
              <div className="w-10 h-10 bg-[#0F172A] dark:bg-white rounded-xl flex items-center justify-center shadow-md group-hover:scale-105 transition-transform duration-300">
                <Zap className="text-emerald-400 dark:text-emerald-600 w-5 h-5 fill-emerald-400 dark:fill-emerald-600" />
              </div>
              <span className="font-extrabold text-2xl tracking-tight text-slate-900 dark:text-white transition-colors">
                MyTax<span className="text-emerald-500">GENIUS</span>
              </span>
            </Link>
            ...
            <nav className="hidden lg:flex items-center space-x-1 flex-1">
              ...
              <div className="relative group">
                <button 
                  ...
                  className="px-2 xl:px-4 py-2.5 rounded-full text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors flex items-center space-x-1 whitespace-nowrap"
                >
                  <span>Features</span>
                  ...
                  {toolsNav.map(item => (
                    <Link
                      ...
                      className={cn(
                        ...
                        location.pathname === item.path ? "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white"
                      )}
                    >
                      {item.name}
                    </Link>
            ...
            <div className="hidden lg:flex items-center space-x-4">
              ...
              <Link
                to="/sme-onboarding"
                className="px-5 py-2.5 rounded-xl text-sm font-extrabold transition-all duration-300 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-white hover:bg-slate-200 dark:hover:bg-slate-700 whitespace-nowrap"
              >
                Hire a Tax Pro
              </Link>
              <Link
                to="/chat"
                className="px-5 py-2.5 rounded-xl text-sm font-extrabold transition-all duration-300 bg-emerald-600 text-white hover:bg-emerald-500 shadow-md whitespace-nowrap flex items-center"
              >
        ...
        <div className={cn(
          "lg:hidden fixed inset-x-0 top-28 bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 shadow-2xl transition-all duration-300 ease-in-out origin-top z-50 p-4",
          ...
          <div className="flex flex-col space-y-2 pb-6 max-h-[calc(100vh-8rem)] overflow-y-auto">
            ...
            <div className="pt-6 mt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col space-y-3">
              ...
              <Link
                to="/chat"
                onClick={() => setIsMobileMenuOpen(false)}
                className="w-full text-center px-4 py-4 rounded-xl text-lg font-extrabold bg-emerald-600 text-white shadow-lg flex items-center justify-center"
              >
                <MessageSquare className="w-5 h-5 mr-2" /> Ask AI Now
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start space-x-2 mb-4">
              ...
              <span className="font-extrabold text-xl text-white">MyTax<span className="text-emerald-400">GENIUS</span></span>
            </div>
          ...
          <div className="flex flex-col items-center md:items-start text-sm">
            ...
            <div className="flex flex-col space-y-4">
              <div className="flex items-start space-x-3 text-slate-400">
                ...
                <span className="font-medium hover:text-emerald-400 cursor-pointer transition-colors">hello@mytaxgenius.com</span>
              </div>
              <div className="flex items-start space-x-3 text-slate-400">
                <Phone className="w-5 h-5 text-emerald-500 shrink-0" />
                <span className="font-medium">+234 (0) 800 TAX GENIUS</span>
              </div>
              <div className="flex items-start space-x-3 text-slate-400">
                <MapPin className="w-5 h-5 text-emerald-500 shrink-0" />
                <span className="font-medium">14 Innovation Drive,<br/>Lekki Phase 1, Lagos, Nigeria</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center md:items-end space-y-4 text-sm font-bold text-center md:text-right">
            ...
            <span className="text-slate-500 font-medium pt-4">&copy; 2026 SMARTMOVE SOLUTIONS LTD.</span>
          </div>
        </div>
      </footer>
      <CookieBanner />
